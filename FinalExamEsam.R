...
tree_pred <- predict(tree_model, training_data, type = "class")
...
tree_cm <- confusionMatrix(data = tree_pred, reference = test_high)
tree_cm
#******************************* PRUNE & PLOT THE TREE *******************************
pruned_cm <- prune.misclass(tree_model, best=7)
plot(pruned_cm, type = "uniform")
pruned_cm
******************* NOTE: USE 7 AS THE BEST MODEL TO PRUNE **************
text(pruned_cm, pretty = 0)
